...
## Data Collection and Manipulation {#sec-data-ingestion}
...
Usage data was provided in a number of report PDFs [@trimet_census] on the TriMet website [@usage_access], which were then ingested and processed to extract the data. Each page in the PDF represented a single route, on a single day, going a single direction. It listed each station along the path, with its usage numbers over the entirety of Spring 2025. TriMet only releases their usage numbers for Spring and Fall, so this is half a year’s worth of data. The PDFs were originally partitioned into Weekday, Saturday, and Sunday documents, but we eventually combined the Saturday and Sunday values into a single “weekend” category.

Information about TriMet schedules was, however, slightly more complicated to gather. Schedules as posted on the TriMet website under their Schedules tab only listed a small subset, about 600, of the over 6,000 station stops that are present in the usage reports. Thankfully, TriMet also has their schedules listed by station, searchable by station ID number, such as the list at [@trimet_stop], which can be accessed from [@stop_page]. This meant that once a list of IDs had been collected from the PDF reports, those IDs could be cross-referenced with the station schedules to obtain the raw schedule data for our many stations of interest, rather than a tenth of them. These schedules were scraped for both a weekday and a weekend day.
...
Stations and routes each have their own table, and are connected by two junction tables, schedule and usage. Each route is defined as a series of stations going a specific direction on a specific day, as mentioned in @sec-terms. The usage table gives a total usage statistic for a specific station on a specific route. Alternatively, you can use the usage_id to select a station-route combination. This is useful within the schedule table, where the same route_id and station_id can have multiple arrival times in a schedule. This ensures third normal form because each table has a primary key and each non-key attribute depends solely on the key.

### Feature Engineering {#sec-features}

Each station has a large number of arrival times from a varying number of routes. However, arrival time alone makes for a poor feature. In pursuit of better features, a number of values were engineered from the schedule data for each station stop: the number of unique routes that stop at the station, the earliest and latest times the station is used, the number of vehicles that stop at the station over the course of the day, and the average number of minutes separating arrivals at the station. These values are all calculated for each station on both weekdays and weekend days. This was done overall by selecting the arrivals at each station on a given day and counting or otherwise manipulating the data into the features of interest. The total usage number sums for each station were also calculated at this stage, since usage information was originally provided at the route level for each station. Distributions of the features engineered in this project can be found in @sec-data. 

## Ethics in Action 

Consider how long you would be willing to stand at a transit station waiting if you watched the previous vehicle stop at the station from across the street and leave without you on it. A survey [@wait-times] taken about transit in Washington, DC found that patrons considered the maximum acceptable wait time to be 20 minutes, but it is common that riders have to spend much longer than that waiting at a station. In this analysis, we engineered a variable that helps with this consideration: the average time between arrivals at each station. We created boxplots of this variable, separated by station type in the TriMet system. This variable was calculated by collecting all the arrivals at a station and averaging the differences between them. This means that stops that are only active during a certain time of day are penalized by this calculation. For example, the WES commuter trains only run during rush hour, with a large gap in the middle of the day, so there is one artificially large value. By combining the WES data with the bus data, this discrepancy has a minimal effect on the data. Overall, however, most routes run consistently throughout the day and the values shown should be reflective of standing at a station waiting for a transit line to arrive as scheduled. It is also worth noting that the data we have access to was schedule data, which does not account for delays or early arrivals. A delayed arrival would, of course, result in an even longer wait time than those calculated in this analysis. 

## Statistical Modeling 

We first decided to use a Poisson statistical model for our data. Poisson is useful for predicting the number of events within a time interval, which we will use to predict the usage of Trimet stations. However, we also have heavily skewed usage data. This is an issue, since Poisson assumes equidispersion: a condition where the mean and variance must be identical. Therefore, we use the Quasi-Poisson model, which accounts for overdispersion by using a dispersion parameter to adjust the variance and standard error. This model uses the following features: Number of routes, earliest arrival, latest arrival, average arrival difference, and day (whether a weekend or weekday). We also used whether the station was a MAX line or not as an interaction variable, interacting with each of the other features. We found that this had the best performance, but the interaction coefficients had some of the highest standard errors and p-values. Therefore, we decided to partition our dataset and create two separate statistical models: one for the higher-usage MAX stations, and the other for the rest of the stations in the dataset.

## Machine Learning 

Because of the left-skewed distributions of the usage values in this dataset (seen in @fig-usage-dist), the usage values had to be log-transformed to act as good target values. While the Poisson model does this automatically, in order for the log-transformed values to be used in our machine learning models we had to engineer one more variable: log-transformed usage. The resulting distribution of this transformation can be seen in @fig-log-dist. There were 1,571 stations that had usage values of 0 and thus produced invalid log-transformed values. These stations were excluded from the regressions undertaken in this project. 

```{r, echo=F, message=F}
#| label: fig-log-dist
#| fig-cap: 'Distributions of log-transformed usage values, by station type. These were the values used as targets for the regressions in this project. 1,571 stations had invalid log-transformed usage values due to an original usage value of 0, and are excluded from this plot.'
