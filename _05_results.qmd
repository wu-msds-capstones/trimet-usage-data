...
20 minutes is often considered a reasonable maximum time to wait at a transit stop between arrivals [@wait-times]. However, it can be seen in @fig-arrival-boxplot that only the MAX lines run every 15-20 minutes regularly, and that only half of the buses by quartile are 25 minutes or less. The third quartile of the bus and WES commuter rail stations goes up over 60 minutes. More than an hour is a long time to wait at a transit stop. 
...
Overall, this model is relatively good at finding a ballpark value, but we would prefer to obtain more predictive features before applying it to any particularly meaningful analysis. 
